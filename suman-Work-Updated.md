...
17. **Section Class Parent Scoping Rule (Preventing CSS Collisions):**
    - **ALWAYS Parent-Scope Selectors:** Every CSS or SCSS selector belonging to a section **MUST use the section's top-level class name as its root/parent prefix** (e.g. `.hm-hero-sec h2`, `.hm-about-sec .contact-info-list li`, `.main-header .navbar-nav .nav-link`).
    - **NEVER Write Unscoped Bare Tags or Shared Classes:** Never write bare element rules like `h2 { ... }` or `p { ... }` inside section stylesheets so that styles never leak or conflict with other sections across the project.
...
[Design Input: Image / Figma]
       │
       ▼ (Enforce 100% Pixel-Perfect Accuracy + Parent Section Scoping + Signature Form Architecture)
 ...
 2. 🎨 Stylesheet Code Block (CSS Default / SCSS on request - NO VARIABLES): All selectors scoped under section parent class + HTML <img> absolute positioning rules + header position: absolute z-index: 999
 3. 📱 Responsive Stylesheet Code Block (responsive.css / SCSS responsive): Media queries scoped under section parent class (1399px @ 86.8%, 1199px inherited, 991px @ 100% + Standardized Heading/Paragraph Scale + mobile relative content / absolute bg img switch + high-contrast text highlights)
...
### 4.1 Header / Navigation Section Blueprint (Scoped Under `.main-header`)
...
### 4.2 Pure Vanilla CSS Blueprint (All Selectors Scoped Under Section Parent Class)

```css
/* ============================================
   STANDALONE HEADER STYLES (SCOPED UNDER .main-header)
   ============================================ */
.main-header {
  position: absolute;
  top: 0;
...
/* ============================================
   HERO SECTION STYLES (SCOPED UNDER .hm-hero-sec)
   ============================================ */
.hm-hero-sec {
  position: relative;
  overflow: hidden;
...
I will immediately generate the **HTML**, **Stylesheet (CSS/SCSS with all selectors scoped under the section parent class)**, **Responsive Stylesheet (`responsive.css`)**, and **JS** blocks for that exact section!
